...
function AddHomeworkModal({ onClose, onAdd, subjects }: {
    ...
    <div className="fixed inset-0 z-50 flex items-end lg:items-center lg:justify-center">
      ...
      <div className="modal-enter relative w-full lg:max-w-md rounded-t-2xl lg:rounded-2xl" style={{ background: N.bg, border: `1px solid ${N.border}`, boxShadow: "0 8px 40px rgba(0,0,0,0.14)" }}>
        ...
        <div className="px-5 py-5 pb-10 lg:pb-5 space-y-3">
          <h3 className="text-[17px] font-bold" style={{ color: N.text }}>New Assignment</h3>
          <input autoFocus type="text" placeholder="Title" value={name} onChange={(e) => setName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && save()}
...
export default function HomeworkPage() {
  ...
  useEffect(() => {
    ...
    const q = activeSemesterId
      ? supabase.from("tasks").select("*").eq("semester_id", activeSemesterId).eq("type", "homework").order("created_at")
      : supabase.from("tasks").select("*").is("semester_id", null).eq("type", "homework").order("created_at");
    ...
    <div className="flex min-h-screen" style={{ background: N.bg }}>
      ...
      <main className="flex-1 lg:ml-56 min-h-screen">
        <div className="lg:hidden flex items-center gap-2 px-4 pt-12 pb-3" style={{ borderBottom: `1px solid ${N.border}` }}>
          ...
          <span className="text-[15px] font-semibold" style={{ color: N.text }}>Homework</span>
        ...
        <div className="max-w-3xl mx-auto px-4 lg:px-12 py-8 pb-28 lg:pb-12">
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            {[{ label: "Pending", value: undone.length, bg: "#FFF7ED", num: "#EA580C" }, { label: "Done", value: done.length, bg: "#F0FDF4", num: "#16A34A" }].map((s) => (
          ...
          <div className="mb-5">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[13px] font-semibold" style={{ color: N.text }}>
                Pending{undone.length > 0 && <span className="ml-1.5 text-[11px] px-1.5 py-0.5 rounded-md tabular-nums" style={{ background: N.hover, color: N.muted }}>{undone.length}</span>}
              </span>
            </div>
      ...
      {modal && <AddHomeworkModal onClose={() => setModal(false)} onAdd={addTask} subjects={subjects} />}
    </div>
  );
}
